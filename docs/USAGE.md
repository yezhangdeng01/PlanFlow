...
> PlanFlow 是一个 Obsidian 计划系统插件：**今日打卡、周/月/年目标、看板与甘特，六视图一体**。数据是纯 Markdown，无需配置任何查询。
...
大多数计划工具把「年度目标、月度计划、每日打卡」做成互不相通的三套数据，靠手动同步。PlanFlow 反其道而行：

![数据流转图](screenshots/flow.svg)

- **数据只产生一次**：年度计划定义在 `年度计划.md` 的 frontmatter，任务写在任务池，打卡勾选写回每日笔记——**没有重复录入**。
- **其余全是查询视图**：本周/本月/年度进度、看板、甘特、统计徽章，全部由日期窗口自动聚合，**零配置**。
...
| **本周 / 本月** | 周期任务预览与进度 | 新建任务、查看完成率 |
| **年度** | 计划总览与目标管理 | 卡片**拖拽排序**、拖边缘调高、量化目标、右键编辑/删除计划 |
| **看板** | 按计划分组的执行面板 | 列头拖拽调宽、按计划过滤 |
| **甘特** | 时间轴排期 | 拖拽调期、查看重叠 |

### 今日视图

![今日视图](screenshots/today.png)
...
![周视图](screenshots/week.png) ![月视图](screenshots/month.png)

按日期窗口自动聚合任务，进度条 + 完成率统计。月/周卡可拖底部调节高度（联动等高）。

### 年度视图

![年度视图](screenshots/year.png)
...
![看板](screenshots/board.png) ![甘特](screenshots/gantt.png)
...
1. **建年度计划**：`raw/计划/2026/年度计划.md`，frontmatter 定义 plans：
...
2. **建任务**：在任务池（`raw/计划/2026/任务.md`）写任务，用 `#计划/阅读` 标签归属，`📅 2026-08-20` 标记截止：

```markdown
- [ ] 读完《认知觉醒》 #计划/阅读 🛫 2026-08-01 📅 2026-08-31
```

3. **打卡**：今日视图勾选即完成，进度自动聚合到周/月/年。

> 根路径 `raw/计划` 可在插件设置中修改。
...
- 界面为中文；数据字段（frontmatter/标签）保持英文，便于未来国际化
